...
const authRes = await fetch(`${testUrl}/authenticate`, {
  method: 'POST',
...
for (let t of tests) {
  ...
  const res = await fetch(`${testUrl}${t.url}`, opts);
  const txt = await res.text();
  console.log(t.method, t.url, "Status:", res.status, txt.substring(0, 100));
}

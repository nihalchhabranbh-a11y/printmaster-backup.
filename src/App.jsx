...
// Bill payment status from bill_payments (or fallback to bill.paid when no payment records)
const getBillPaymentInfo = (bill, billPayments) => {
  ...
  const fromPayments = payments.length > 0;
  const remaining = fromPayments ? Math.max(0, total - paidAmount) : (bill.paid ? 0 : total);
  const status = fromPayments
    ? (paidAmount <= 0 ? "Unpaid" : paidAmount >= total ? "Paid" : "Partially Paid")
    : (bill.paid ? "Paid" : "Unpaid");
  const isPaid = fromPayments ? paidAmount >= total : !!bill.paid;
  const displayPaidAmount = fromPayments ? paidAmount : (bill.paid ? total : 0);
...
.sidebar{position:fixed;left:0;top:0;bottom:0;width:var(--sw);background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;z-index:100;transition:transform var(--t),background var(--t);overflow-y:auto;box-shadow:2px 0 12px rgba(0,0,0,.04)}
...
.nav-item{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:var(--radius-sm);cursor:pointer;font-size:.875rem;font-weight:500;color:var(--text2);margin:2px 4px;transition:transform var(--t-fast),background var(--t),color var(--t)}
.nav-item:hover{background:var(--surface2);color:var(--text);transform:translateX(6px) scale(1.03)}
.nav-item:active{transform:translateX(3px) scale(0.99)}
.nav-item.active{background:var(--accent-soft);color:var(--accent);font-weight:600;box-shadow:inset 0 0 0 1px rgba(37,99,235,.15)}
...
.user-chip{display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:var(--radius-sm);background:var(--surface2);cursor:pointer;transition:transform var(--t-fast),background var(--t)}
.user-chip:hover{background:var(--border);transform:translateY(-1px)}
...
.landing{min-height:100vh;background:linear-gradient(180deg,#040816 0%,#0a1023 42%,#0c1330 100%);color:#eef2ff}
.landing a{color:inherit}
.landing .container{max-width:1160px;margin:0 auto;padding:0 18px}
.landing-top{position:sticky;top:0;z-index:20;backdrop-filter:blur(10px);background:rgba(5,8,22,.55);border-bottom:1px solid rgba(148,163,184,.14)}
.landing-nav{display:flex;align-items:center;justify-content:space-between;padding:14px 0}
.brandmark{display:flex;align-items:center;gap:10px;font-weight:900;letter-spacing:.2px}
.brandmark-badge{width:38px;height:38px;border-radius:12px;background:linear-gradient(135deg,#4f67ff,#7c3aed);display:flex;align-items:center;justify-content:center;box-shadow:0 18px 42px rgba(79,103,255,.25)}
.landing-actions{display:flex;gap:10px;align-items:center}
.landing-btn{border:1px solid rgba(148,163,184,.24);background:rgba(15,23,42,.72);padding:10px 14px;border-radius:12px;font-weight:800;cursor:pointer;color:#f8fafc}
.landing-btn.primary{border-color:transparent;background:linear-gradient(135deg,#5b61ff,#8447ff);box-shadow:0 18px 42px rgba(79,103,255,.22);color:#fff}
.landing-btn:hover{filter:brightness(1.08)}
.hero{padding:28px 0 18px}
.hero-banner{border-radius:28px;overflow:hidden;border:1px solid rgba(148,163,184,.16);background:
  linear-gradient(180deg,rgba(2,6,23,.35),rgba(2,6,23,.65)),
  url("/hero-bg.svg");
  background-size:cover;
  background-position:center;
  box-shadow:0 40px 120px rgba(0,0,0,.55)}
.hero-inner{padding:46px 26px}
.hero-grid{display:grid;grid-template-columns:1.05fr .95fr;gap:24px;align-items:center}
.hero h1{font-size:48px;line-height:1.03;margin:0 0 12px;color:#fff;letter-spacing:-.03em;text-shadow:0 18px 80px rgba(0,0,0,.55)}
.hero p{margin:0 0 18px;color:rgba(239,244,255,.92);font-size:15px;line-height:1.65;max-width:52ch}
.hero-cta{display:flex;gap:12px;flex-wrap:wrap;margin-top:18px}
.hero-kpis{display:flex;gap:12px;flex-wrap:wrap;margin-top:20px}
.kpi{flex:1;min-width:180px;padding:14px 14px;border-radius:18px;border:1px solid rgba(255,255,255,.14);background:rgba(15,23,42,.22);backdrop-filter:blur(10px)}
.kpi .n{font-weight:900;color:#fff;font-size:18px}
.kpi .t{color:rgba(236,241,255,.82);font-size:12px;margin-top:4px}
.hero-device{position:relative;border-radius:24px;border:1px solid rgba(255,255,255,.16);background:rgba(2,6,23,.28);backdrop-filter:blur(10px);padding:14px;box-shadow:0 26px 80px rgba(0,0,0,.45)}
.hero-device-top{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px}
.hero-chip{display:inline-flex;align-items:center;gap:8px;padding:8px 10px;border-radius:999px;border:1px solid rgba(255,255,255,.16);background:rgba(2,6,23,.35);color:rgba(244,247,255,.96);font-weight:900;font-size:12px}
.hero-screen{border-radius:18px;overflow:hidden;border:1px solid rgba(148,163,184,.18);background:rgba(15,23,42,.22)}
.hero-screen img{width:100%;height:auto;display:block}
.hero-screen-overlay{position:absolute;inset:auto 18px 18px auto;padding:10px 12px;border-radius:16px;border:1px solid rgba(255,255,255,.14);background:rgba(15,23,42,.22);backdrop-filter:blur(10px);font-weight:900;color:#fff;box-shadow:0 18px 60px rgba(0,0,0,.35)}
.section{padding:26px 0}
.section h2{margin:0 0 10px;font-size:26px;color:#fff}
.muted{color:rgba(237,242,255,.86);line-height:1.7}
.feature-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:16px}
.feature{padding:16px;border-radius:16px;border:1px solid rgba(148,163,184,.16);background:rgba(15,23,42,.28)}
.feature .h{font-weight:900;color:#fff;margin-bottom:6px}
.feature .d{color:rgba(236,241,255,.82);font-size:13px;line-height:1.6}
.pricing{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:16px}
.plan{padding:18px;border-radius:18px;border:1px solid rgba(148,163,184,.16);background:rgba(15,23,42,.28)}
.plan.best{border-color:rgba(99,102,241,.55);box-shadow:0 26px 70px rgba(79,103,255,.14)}
.plan .name{font-weight:900;color:#fff}
.plan .price{font-size:28px;font-weight:900;color:#fff;margin-top:8px}
.plan ul{margin:14px 0 0;padding:0 0 0 16px;color:rgba(239,244,255,.9);font-size:13px;line-height:1.7}
.faq{margin-top:14px;display:grid;gap:10px}
.faq details{border:1px solid rgba(148,163,184,.16);background:rgba(15,23,42,.24);border-radius:16px;padding:12px 14px}
.faq summary{cursor:pointer;font-weight:900;color:#fff}
.footer{padding:26px 0;border-top:1px solid rgba(148,163,184,.14);color:rgba(236,241,255,.78);font-size:12px}
...
@media(max-width:768px){
  ...
  .landing-nav{flex-direction:column;align-items:stretch;gap:14px}
  .landing-actions{width:100%}
  .landing-actions .landing-btn{flex:1;display:flex;align-items:center;justify-content:center;min-height:46px}
  .hero{padding:18px 0 12px}
  .hero-banner{border-radius:22px}
  .hero-inner{padding:22px 16px}
  .hero-grid{gap:18px}
  .hero h1{font-size:2.25rem;line-height:1.02}
  .hero p{font-size:.98rem;max-width:none}
  .hero-cta{flex-direction:column}
  .hero-cta .landing-btn{width:100%;display:flex;justify-content:center;min-height:48px}
  .hero-kpis{gap:10px}
  .kpi{min-width:0;padding:12px}
  .hero-device{padding:10px;border-radius:20px}
  .hero-device-top{flex-direction:column;align-items:flex-start}
  .hero-screen-overlay{position:static;margin-top:10px;display:inline-flex}
  .section{padding:18px 0}
  .section h2{font-size:1.8rem}
  .feature,.plan,.faq details{padding:14px}
...
@media(max-width:480px){
  .brandmark{align-items:flex-start}
  .brandmark-badge{width:34px;height:34px;border-radius:10px}
  .hero h1{font-size:1.95rem}
  .hero-chip{font-size:11px;padding:7px 9px}
  .hero-screen-overlay{font-size:.9rem;width:100%;justify-content:center;text-align:center}
...
export default function App() {
        ...
        if (parsed && parsed.username && parsed.role) {
          ...
          fetch('http://127.0.0.1:7590/ingest/7bb49d42-87e4-4411-a67b-74994a07da53', {
            ...
            body: JSON.stringify({
              ...
              runId: 'initial',
              hypothesisId: 'H3',
              location: 'App.jsx:restoreUser',
              message: 'Restoring user from localStorage',
              data: { user: { id: parsed.id, username: parsed.username, role: parsed.role } },
              timestamp: Date.now(),
            }),
          }).catch(() => {});
          // #endregion
          ...
          fetch('http://127.0.0.1:7590/ingest/7bb49d42-87e4-4411-a67b-74994a07da53', {
            ...
            body: JSON.stringify({
              ...
              runId: 'initial',
              hypothesisId: 'H5',
              location: 'App.jsx:defaultPageForRole',
              message: 'Setting default page after restore',
              data: { role: parsed.role, nextPage },
              timestamp: Date.now(),
            }),
          }).catch(() => {});
          // #endregion
  ...
  }, [bills, tasks, vendorBills, user?.organisationId, dbLoading]);
  ...
  const totalRevenue = hasBillPayments
    ...
    : paidBills.reduce((s, b) => s + (b.total || 0), 0);
  ...
  const navItems = !user ? [] : isSuperAdmin ? [
    ...
    { id: "products", label: "Products / Services", icon: "billing" },
  ...
  useEffect(() => {
    if (!user) return;
    const allowed = new Set(navItems.map(x => x.id));
    if (!allowed.has(page)) {
          ...
          onLogin={u => {
            ...
            fetch('http://127.0.0.1:7590/ingest/7bb49d42-87e4-4411-a67b-74994a07da53', {
              ...
              body: JSON.stringify({
                ...
                hypothesisId: 'H5',
                location: 'App.jsx:defaultPageForRole',
                message: 'Setting default page after login',
                data: { role: simpleUser.role, nextPage },
                timestamp: Date.now(),
              }),
            }).catch(() => {});
            // #endregion
  ...
  const pages = {
    dashboard: <DashboardPage bills={bills} tasks={tasks} workers={workers} vendors={vendors} billPayments={billPayments} totalRevenue={totalRevenue} totalPending={totalPending} paidBills={paidBills} unpaidBills={unpaidBills} pendingTasks={pendingTasks} completedTasks={completedTasks} setPage={setPage} brand={brand} />,
    billing: <SimpleBillingPage bills={bills} setBills={setBills} billPayments={billPayments} setBillPayments={setBillPayments} showToast={showToast} customers={customers} setCustomers={setCustomers} brand={brand} setBrand={setBrand} user={user} products={products} />,
    ...
    workers: <WorkersPage workers={workers} setWorkers={setWorkers} tasks={tasks} showToast={showToast} user={user} />,
    vendors: <VendorsPage vendors={vendors} setVendors={setVendors} vendorBills={vendorBills} setVendorBills={setVendorBills} vendorPayments={vendorPayments} setVendorPayments={setVendorPayments} showToast={showToast} brand={brand} user={user} />,
    ...
    customers: <EnhancedCustomersPage customers={customers} setCustomers={setCustomers} bills={bills} showToast={showToast} user={user} />,
  ...
  const titles = { dashboard:"Dashboard", billing:"Billing", products:"Products / Services", tasks:"Task Management", workers:"Workers", vendors:"Vendors", payments:"Payments", purchases:"Purchases", customers:"Customers", reports:"Revenue Reports", "org-approvals":"Org Approvals", admin:"Admin Panel", settings:"Settings", "worker-dashboard":"My Dashboard", "worker-tasks":"My Tasks", "vendor-dashboard":"Vendor Dashboard", "vendor-tasks":"My Tasks", "vendor-bills":"Create Bill" };
                ...
                onClick={() => {
                  ...
                  fetch('http://127.0.0.1:7590/ingest/7bb49d42-87e4-4411-a67b-74994a07da53', {
                    ...
                    body: JSON.stringify({
                      ...
                      runId: 'initial',
                      hypothesisId: 'H2',
                      location: 'App.jsx:navigation',
                      message: 'Navigation item clicked',
                      data: { targetPage: item.id, currentPage: page, userRole: user.role },
                      timestamp: Date.now(),
                    }),
                  }).catch(() => {});
                  // #endregion
...
const getBillItems = (bill) => {
  ...
  return [{
    desc: bill.desc || bill.description || "",
    size: bill.size || "",
...
// ── Add Payment Modal ─────────────────────────────────────────────────────────
function AddPaymentModal({ bill, billPayments, onAdd, onClose }) {
...
function PublicInvoicePage({ invId }) {
  ...
  const invLink = `${window.location.origin}?inv=${bill.id}`;
  const invQR = qrImgUrl(invLink, 90);
  const payInfo = getBillPaymentInfo(bill, []);
  const handleDownload = () => {
    if (invRef.current) downloadInvoiceElement(invRef.current, `${bill.id || "invoice"}.pdf`);
  };
  const handlePrint = () => {
...
// ── Workers ───────────────────────────────────────────────────────────────────
function WorkersPage({ workers, setWorkers, tasks, showToast, user }) {
  const [showModal, setShowModal] = useState(false);
  ...
  const add = async () => {
    ...
    const saved = await db.addWorker({ role:"worker", ...form, organisationId: user?.organisationId });
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:14}}>
        {workers.map(w=>{
          ...
          <div key={w.id} className="card" style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",gap:10}}>
            ...
            <div style={{fontSize:".8rem",color:"var(--text2)",fontWeight:600}}>Tasks completed: {completed}</div>
            <div className="flex gap-2">
...
// ── Vendors ───────────────────────────────────────────────────────────────────
function VendorsPage({ vendors, setVendors, vendorBills, setVendorBills, vendorPayments, setVendorPayments, showToast, brand, user }) {
  const [showModal, setShowModal] = useState(false);

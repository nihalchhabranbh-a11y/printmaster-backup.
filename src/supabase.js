...
export const db = {
  ...
  async loginWithCredentials(username, password, superAdminPassword) {
    ...
    // Org admins
    {
      const { data, error } = await supabase
        ...
        .limit(1)
        .maybeSingle();
      if (!error && data && allowedOrg(data.organisations)) {
        return {
          id: data.id,
  ...
  async addPurchase(purchase) {
    ...
    if (purchase.organisationId) row.organisation_id = purchase.organisationId;
    const { data, error } = await supabase
      .from("purchases")
      .insert([row])
      .select()
  ...
  async saveBrand(brand, organisationId) {
    ...
    const payload = {
      ...
      invoice_counter: brand.invoiceCounter ?? null,
    };
